...
namespace SteerLib {
	...
	class STEERLIB_API CameraView {
		public:
			...
			CameraView() {
				...
				targetTangent = Util::Vector(0.0f, 0.0f, 0.0f);
				targetTime = 0;
			}
	...
	class STEERLIB_API Camera
		...
		public:
			...
			void addControlPoints(const std::vector<Util::CurvePoint>& controlPoints) { curve.addControlPoints(controlPoints); }
			void animate(float timeStamp, float dt, unsigned int frameNumber);
